...
export type CustomFunctionDefaultValues = Record<
  string,
  boolean | string | number | Record<string, unknown> | Array<string>
>;

export interface PropertyCustomFunctionComponentProps<
  FormikValues,
  CustomComponentValues = CustomFunctionDefaultValues
> {
  className?: string;
...
export type PropertyCustomFunctionComponent<
  T extends PropertyCustomFunctionComponentProps<any>
...
export const CustomComponentWrapper: React.FunctionComponent<
...
const withCustomComponentWrapper = function <
  V,
  T extends PropertyCustomFunctionComponentProps<any> = PropertyCustomFunctionComponentProps<V>
>(
  InnerComponent: React.FunctionComponent<any>
): PropertyCustomFunctionComponent<T> {
  const displayName = `withCustomComponentWrapper(${
    InnerComponent.displayName || InnerComponent.name
  })`;
  const WrappedComponent: React.FunctionComponent<any> = (
    props: PropertyCustomFunctionComponentProps<unknown>
  ): JSX.Element => {
    const { property, hasChanged, _context, formik, disabled } = props;

    const isListValue =
      !formik && _context === CustomFunctionComponentContext.read;
    if (isListValue) {
