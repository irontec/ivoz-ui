...
export default class ApiSpecParser {
        ...
        for (const method in paths) {
          ...
          if (method !== 'get') {
            response[modelName].actions[method][modelVariant] = {
              ...
              parameters:
                modelVariantPaths[Object.keys(modelVariantPaths)[0]].parameters,
  ...
  private getItemPaths(paths: any) {
    return Object.keys(paths).filter((key: string) => {
      return (
        paths[key]?.responses['200']?.schema?.$ref ||
        paths[key]?.produces.includes('application/octet-stream')
      );
    });
  }

  public getEntityBasePath(entityNameRoot: string): string {
    const underScored = entityNameRoot.replace(
      /([A-Z])/g,
      (upperString: string) => {
  ...
  private getEntityPaths(
    entityNameRoot: string,
    apiSpec: OpenApiSpecInterface
  ): OpenApiPaths {
  ...
  private filterByResponseSchema(
    ...
    const isFileDownload =
      action.produces && action.produces.includes('application/octet-stream');
    ...
    if (isFileDownload) {
      return Object.values(action.responses)
        ...
        .map(
          (response: any) =>
            response.description.replace('#/definitions/', '') ?? ''
        )
        .map((description: string) => {
          const modelMatch =
            description.toLowerCase() === entityNameRoot.toLowerCase();
  ...
  private isUploadAction(action: any): boolean {
    const fileParams: Array<any> = action.parameters.filter((param: any) => {
