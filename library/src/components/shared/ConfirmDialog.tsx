...
import React, { ChangeEventHandler, ComponentType, forwardRef, useCallback, useState } from 'react';
...
export default function ConfirmDialog(props: ConfirmDialogProps): JSX.Element {
  ...
  return (
    ...
    >
      <DialogTitle id='alert-dialog-slide-title'>{_('Remove element')}</DialogTitle>
      <DialogContent>
        ...
        {doubleCheck && (
          <StyledDialogContentText id='alert-dialog-double-check-description'>
            {_('Please type the item name, as shown in bold font above, to continue')}
      ...
      <DialogActions>
        ...
        <Button disabled={!sumbitEnabled} onClick={handleApply}>{_('Delete')}</Button>
      </DialogActions>
    </Dialog>
  );
}
